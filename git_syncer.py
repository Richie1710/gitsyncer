...
class GitSyncer:
    ...
    def get_source_latest_commit_sha(self):
        """
        Gets the SHA of the latest commit in the source repository.
    ...
    def clone_source(self):
        """
        Clones the source repository to the local path.
        """
    ...
    def pull_source(self):
        """
        Pulls the latest changes from the source repository.
        """
    ...
    def push_target(self):
        """
        Pushes the cloned source repository to the target repository.
        """
    ...
    def get_target_pull_requests(self):
        """
        Gets the list of pull requests from the target repository.
    ...
    def check_for_new_commit(self):
        """
        Checks if there is a new commit in the source repository.
        If a new commit is detected, handle_new_commit method is called.
    ...
    def handle_new_commit(self, commit_sha: str):
        """
        Handles the process when a new commit is detected.
    ...
    def check_if_pr_exists(self, commit_sha: str, pr_list: list) -> bool:
        """
        Checks if a pull request for the given commit SHA already exists.
    ...
    def push_to_target_branch(self, branch_name: str):
        """
        Pushes the new commit to the created branch in the target repository.
    ...
    def create_pr_in_target(self, branch_name: str):
        """
        Creates a new pull request in the target repository.
    ...
    def get_target_repository(self, owner: str, repo_name: str, auto_create: bool = False):
        try:
            ...
            logging.info(f"Zugriff auf das Repository: {repo['full_name']}")
            logging.info(f"Repository URL: {repo['html_url']}")
            logging.info(f"Besitzer: {repo['owner']['login']}")
            logging.info(f"Erstellt am: {repo['created_at']}")
            logging.info(f"Aktualisiert am: {repo['updated_at']}")
            return repo
            ...
            if e.response.status_code == 404 and auto_create:
                ...
                repo = self.target_api.get_repository(owner, repo_name)
                logging.info(f"Zugriff auf das neu erstellte Repository: {repo['full_name']}")
                logging.info(f"Repository URL: {repo['html_url']}")
                logging.info(f"Besitzer: {repo['owner']['login']}")
                logging.info(f"Erstellt am: {repo['created_at']}")
                logging.info(f"Aktualisiert am: {repo['updated_at']}")
    ...
    def sync_source_to_target(self, owner: str, repo: str):
        # Ihr vorheriger Code bis zum Klonen...
    ...
    def run(self):
        """
        Starts the syncing process and keeps it running until stopped.
        """
